...
object Wenao1PercentWealthAggregator {
  // So the logic is that country wealth brackets show what percentage country X makes up of what wealth bracket
  ...
  def process(countryData: Vector[CountryData]): Long = {
    ...
    val onePercentTotalWealth = worldTotalWealth * .5013
